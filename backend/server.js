...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 5 — Poster renderer  (mirrors poster_renderer.py exactly)
//
// Requires the `@napi-rs/canvas` npm package (added to package.json).
// Layout modes:
//   A — backdrop only  (800 × ~450, vignette)
//   B — backdrop + logo in dark strip below  (Done without Edit)
//   C — backdrop + logo overlaid with editor transform  (Done after Edit)
//
// All modes produce an 800 px wide JPEG at quality 93.
// ═══════════════════════════════════════════════════════════════════════════

const POSTER_W          = 800;
const STRIP_BG_CSS      = 'rgb(10,10,14)';
const STRIP_MIN_H       = 120;
const STRIP_PADDING     = 20;
const LOGO_MAX_W_RATIO  = 0.70;
const LOGO_MAX_H        = 90;
const OVL_MAX_W_RATIO   = 0.65;
const OVL_BOTTOM_MRG    = 0.08;

// Lazy-load canvas to avoid crash when the package is missing
let _napi = null;
function getCanvas() {
  ...
  try {
    ...
    _napi = null;
    console.warn('⚠️  @napi-rs/canvas not installed — poster rendering unavailable. Run: npm install @napi-rs/canvas');
  }
  return _napi;
}
...
/**
 * Render poster, return JPEG Buffer.
 * @param {object} opts
 * @param {string}  opts.backdropUrl  - TMDB backdrop URL (HD preferred)
 * @param {string}  [opts.logoUrl]    - TMDB transparent PNG logo URL or null
 * @param {object}  [opts.transform]  - Editor transform {x,y,scale,rotation} or null
 * @param {boolean} [opts.vignette]   - Add soft bottom fade (default true)
 */
async function renderPoster({ backdropUrl, logoUrl = null, transform = null, vignette = true }) {
  const C = getCanvas();
  if (!C) throw new Error('@napi-rs/canvas is not installed. Run: npm install @napi-rs/canvas');

  // ── Fetch images ───────────────────────────────────────────────────────
  const bdBuf = await fetchImageBuffer(backdropUrl);
  const bdImg = await C.loadImage(bdBuf);

  let logoImg = null;
  if (logoUrl) {
    try {
      const lgBuf = await fetchImageBuffer(logoUrl);
      logoImg     = await C.loadImage(lgBuf);
    } catch (e) {
      console.warn('[render] Logo fetch failed, continuing without logo:', e.message);
    }
  }

  // ── Scale backdrop: width = POSTER_W, height proportional ─────────────
  const bdH = Math.round((bdImg.height / bdImg.width) * POSTER_W);

  // ── Choose mode and calculate total canvas height ─────────────────────
  let totalH, stripH = 0;
  if (logoImg && transform !== null) {
    // Mode C — logo overlaid on backdrop
    totalH = bdH;
  } else if (logoImg) {
    // Mode B — logo in strip below backdrop
    const lscale = Math.min((POSTER_W * LOGO_MAX_W_RATIO) / logoImg.width, LOGO_MAX_H / logoImg.height, 1.0);
    const slh    = Math.max(1, Math.round(logoImg.height * lscale));
    stripH = Math.max(slh + STRIP_PADDING * 2, STRIP_MIN_H);
    totalH = bdH + stripH;
  } else {
    // Mode A — backdrop only
    totalH = bdH;
  }

  const canvas = C.createCanvas(POSTER_W, totalH);
  const ctx    = canvas.getContext('2d');

  // ── Draw backdrop ──────────────────────────────────────────────────────
  ctx.drawImage(bdImg, 0, 0, POSTER_W, bdH);

  // ── Vignette: soft dark fade at the bottom of the backdrop ────────────
  //    Mode A uses 8 % fade; Mode C uses 20 % fade (more cinematic).
  //    Mode B gets no vignette — logo strip provides natural separation.
  if (vignette && !(logoImg && transform === null)) {
    const fadeRatio = (logoImg && transform !== null) ? 0.20 : 0.08;
    const fadeH     = Math.floor(bdH * fadeRatio);
    const grad      = ctx.createLinearGradient(0, bdH - fadeH, 0, bdH);
    grad.addColorStop(0, 'rgba(0,0,0,0)');
    grad.addColorStop(1, 'rgba(0,0,0,0.63)');
    ctx.fillStyle = grad;
    ctx.fillRect(0, bdH - fadeH, POSTER_W, fadeH);
  }

  // ── Mode B: dark strip with centred logo ──────────────────────────────
  if (logoImg && transform === null) {
    ctx.fillStyle = STRIP_BG_CSS;
    ctx.fillRect(0, bdH, POSTER_W, stripH);
    const lscale = Math.min((POSTER_W * LOGO_MAX_W_RATIO) / logoImg.width, LOGO_MAX_H / logoImg.height, 1.0);
    const slw = Math.max(1, Math.round(logoImg.width  * lscale));
    const slh = Math.max(1, Math.round(logoImg.height * lscale));
    const lx  = Math.floor((POSTER_W - slw) / 2);
    const ly  = bdH + Math.floor((stripH - slh) / 2);
    ctx.drawImage(logoImg, lx, ly, slw, slh);
  }

  // ── Mode C: logo overlaid using editor transform ───────────────────────
  if (logoImg && transform !== null) {
    const cx       = parseFloat(transform.x        ?? 0.5);
    const cy       = parseFloat(transform.y        ?? 0.80);
    const scalePct = parseFloat(transform.scale    ?? OVL_MAX_W_RATIO);
    const rotation = parseFloat(transform.rotation ?? 0);
    const targetW  = Math.max(10, Math.floor(POSTER_W * scalePct));
    const targetH  = Math.max(1, Math.round(logoImg.height * targetW / logoImg.width));
    const px = cx * POSTER_W;
    const py = cy * bdH;
    ctx.save();
    ctx.translate(px, py);
...
// ── GET /api/poster/images  (TMDB images for the poster editor) ────────────
app.get('/api/poster/images', async (req, res) => {
  const series_id = (req.query.series_id || '').trim();
  if (!series_id) return res.status(400).json({ error: 'Missing series_id' });
  if (!TMDB_API_KEY && !TMDB_READ_ACCESS_TOKEN) return res.status(500).json({ error: 'TMDB not configured' });
  try {
    const series = await db.collection('series').findOne({ _id: series_id });
    if (!series) return res.status(404).json({ error: 'Series not found' });
    // Resolve TMDB ID: prefer stored "tmdb_XXXXX" imdb_id field, else title search
    let tmdb_id = null;
    const imdbF = (series.imdb_id || '').trim();
    if (imdbF.startsWith('tmdb_')) {
      tmdb_id = imdbF.slice(5);
    } else {
      try {
        const sd = await tmdbFetch('/search/tv', { query: series.title || '', language: 'en-US' });
        if ((sd.results || []).length > 0) tmdb_id = String(sd.results[0].id);
      } catch (_) {}
    }
    if (!tmdb_id) return res.json({ title: series.title || '', series_id, backdrops: [], logos: [], warning: 'Could not resolve TMDB ID' });
    let imgData;
    try { imgData = await tmdbFetch(`/tv/${tmdb_id}/images`); }
    catch (_) { return res.json({ title: series.title || '', series_id, backdrops: [], logos: [] }); }
    const backdrops = (imgData.backdrops || []).slice(0, 20).map(img => ({
      url:      TMDB_W500  + img.file_path,
      url_hd:   TMDB_W1280 + img.file_path,
      url_orig: TMDB_ORIG  + img.file_path,
    }));
    const logos = (imgData.logos || []).slice(0, 15)
      .filter(img => img.file_path && img.file_path.endsWith('.png'))
      .map(img => ({ url: TMDB_W500 + img.file_path, url_hd: TMDB_ORIG + img.file_path }));
    res.json({ title: series.title || '', series_id, backdrops, logos });
  } catch (e) { console.error('GET /api/poster/images:', e); res.status(500).json({ error: String(e) }); }
});

// ── POST /api/poster/save  (render → upload → MongoDB) ────────────────────
//
// Body JSON:
//   series_id    (string, required) — matches series._id in MongoDB
//   backdrop_url (string, required) — TMDB HD backdrop URL
//   logo_url     (string|null)      — TMDB transparent PNG logo URL
//   transform    (object|null)      — editor transform {x,y,scale,rotation}
//   init_data    (string)           — Telegram WebApp.initData (ignored here,
//                                     kept for API compatibility with bot)
//
// Response JSON on success:
//   { success: true, poster_url: "https://…", series_id: "…" }
//
app.post('/api/poster/save', async (req, res) => {
  const body         = req.body || {};
  const series_id    = (body.series_id    || '').trim();
  const backdrop_url = (body.backdrop_url || '').trim();
  ...
  if (!backdrop_url) return res.status(400).json({ error: 'backdrop_url is required' });

  // Verify series exists
  let series;
  try {
    series = await db.collection('series').findOne({ _id: series_id });
    if (!series) return res.status(404).json({ error: `Series not found: ${series_id}` });
  } catch (e) {
    console.error('[poster/save] DB lookup:', e);
    return res.status(500).json({ error: 'Database error: ' + e.message });
  }

  // Render poster to JPEG Buffer
  let jpegBuffer;
  try {
    jpegBuffer = await renderPoster({ backdropUrl: backdrop_url, logoUrl: logo_url, transform, vignette: true });
  } catch (e) {
    console.error('[poster/save] Render error:', e);
    return res.status(500).json({ error: 'Render failed: ' + e.message });
  }

  // Upload (ImgBB → Catbox fallback)
  let poster_url;
  try {
    poster_url = await uploadPosterWithFallback(jpegBuffer);
    if (!poster_url) return res.status(500).json({ error: 'All upload services failed. Check IMGBB_API_KEY / CATBOX_API_KEY.' });
  } catch (e) {
    console.error('[poster/save] Upload error:', e);
    return res.status(500).json({ error: 'Upload error: ' + e.message });
  }

  // Persist URL to MongoDB (same field the bot writes to)
  try {
    ...
    console.error('[poster/save] DB write:', e);
