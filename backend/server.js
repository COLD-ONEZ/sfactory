...
app.get('/api/poster/images', async (req, res) => {
  ...
  try {
    ...
    const headers  = TMDB_KEY ? { Authorization: `Bearer ${TMDB_KEY}` } : {};

    const imgRes  = await fetch(`https://api.themoviedb.org/3/tv/${tmdb_id}/images?include_image_language=en,null`, { headers });
