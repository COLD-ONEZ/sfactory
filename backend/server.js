...
app.get('/api/poster/images', async (req, res) => {
  ...
  try {
    ...
    // Resolve TMDB ID — same logic as SeriesBoT's _resolve_tmdb_id
    let tmdb_id = null;
    const raw_id = series.imdb_id || '';
    if (raw_id.startsWith('tmdb_')) {
      ...
      const sRes = await fetch(tmdbUrl('/search/tv', { query: title }), { headers: tmdbHeaders() });
      if (sRes.ok) {
        const sData = await sRes.json();
        const first = (sData.results || [])[0];
        if (first) tmdb_id = String(first.id);
      }
    }

    if (!tmdb_id) return res.json({ title, backdrops: [], logos: [] });

    // Fetch images — no language filter, same as SeriesBoT
    const imgRes  = await fetch(tmdbUrl(`/tv/${tmdb_id}/images`), { headers: tmdbHeaders() });
    if (!imgRes.ok) return res.json({ title, backdrops: [], logos: [] });
    const imgData = await imgRes.json();

    const BASE_W500 = 'https://image.tmdb.org/t/p/w500';
    const BASE_HD   = 'https://image.tmdb.org/t/p/w1280';
    const BASE_ORIG = 'https://image.tmdb.org/t/p/original';

    const backdrops = (imgData.backdrops || []).slice(0, 20)
      .filter(b => b.file_path)
      .map(b => ({
        url:    BASE_W500 + b.file_path,
        url_hd: BASE_HD   + b.file_path,
      }));

    const logos = (imgData.logos || []).slice(0, 15)
      .filter(l => l.file_path && l.file_path.endsWith('.png'))
      .map(l => ({
        url:    BASE_W500 + l.file_path,
        url_hd: BASE_ORIG + l.file_path,
      }));
...
// ── GET /api/proxy/image  (proxy TMDB images to avoid canvas CORS issues) ───
