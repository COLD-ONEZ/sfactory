 ...
 *   type       — 'file' | 'batch'
 *
 * The bot handles start= params in plain format:
 *   get_{channel_id}_{msg_id}_{msg_id}          → single episode file (same ID twice)
 *   get_{channel_id}_{first_msg_id}_{last_msg_id} → season batch
 *
 * channel_id is the absolute value of MAIN_DB_CHANNEL (without -100 prefix).
 */
app.get('/api/link', async (req, res) => {
  try {
    ...
    if (type === 'file' && episode_id) {
      // Single episode file
      // Bot saves link as: get_{channel_id}_{msg_id}_{msg_id}  (same ID for start and end)
      ...
      // If the bot already stored a file_link, extract the start param directly from it
      if (eq.file_link) {
        const match = eq.file_link.match(/[?&]start=([^&]+)/);
        if (match) {
          startParam = match[1];
        }
      }

      // Fallback: build from msg_id + db_channel_id using the correct get_ format
      if (!startParam) {
        // db_channel_id is stored as the full negative id (e.g. -1001234567890)
        // strip the leading -100 to get the plain channel id used in the link
        const channelId = String(eq.db_channel_id || '').replace(/^-100/, '');
        startParam = `get_${channelId}_${eq.msg_id}_${eq.msg_id}`;
      }

    } else {
      // Batch (full season)
      // Bot saves link as: get_{channel_id}_{first_msg_id}_{last_msg_id}
      ...
      if (q.batch_link) {
        // Extract the start= param from the stored t.me deep-link
        // Stored format: https://t.me/{bot}?start=get_{channel_id}_{first}_{last}
        const match = q.batch_link.match(/[?&]start=([^&]+)/);
        if (match) {
          startParam = match[1];
        } else {
          // Already a raw start param (no URL wrapper)
          startParam = q.batch_link;
        }
      } else if (q.first_msg_id && q.last_msg_id) {
        // Build from stored range using the correct get_ format
        const channelId = String(q.db_channel_id || '').replace(/^-100/, '');
        startParam = `get_${channelId}_${q.first_msg_id}_${q.last_msg_id}`;
      } else {
...
// GET /api/ads/active — return non-expired ads from shared MongoDB ads collection
// (Ads are SET by the Telegram bot via /setad command; this site reads the same DB)
app.get('/api/ads/active', async (req, res) => {
  try {
    const now  = new Date();
    const docs = await db.collection('ads').find({}).toArray();
    const ads  = docs
      .filter(doc => !doc.expires_at || new Date(doc.expires_at) > now)
      .map(doc => ({
        slot:      doc._id,           // "banner" | "infeed" | "sticky"
        type:      doc.type,          // "photo"  | "video"
        file_url:  doc.file_url,      // served by bot's aiohttp /api/ad-media/
        click_url: doc.click_url || null,
      }));
    res.json({ ok: true, ads });
  } catch (err) {
    console.error('GET /api/ads/active:', err);
    res.status(500).json({ ok: false, ads: [] });
  }
});
...
// Poster editor API — served by the bot's aiohttp server, not sfactory.
// Without these, fetch('/api/poster/...') inside poster_editor.html hits the SPA
// fallback and gets index.html back → "Unexpected token '<' is not valid JSON".
app.all('/api/poster/images',  (req, res) => proxyBotApi(req, res));
app.all('/api/poster/save',    (req, res) => proxyBotApi(req, res));
